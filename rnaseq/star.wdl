task star {
    ...
    command {
        ...
        mkdir star_index
        tar -xvf ${star_index} --directory=star_index # --strip-components=1 ## apply if tarball was created around entire directory, e.g. tar -tvf file.tar contains the directory 'star_index'

        mkdir star_out
        # placeholders for optional outputs
        echo $(date +"[%b %d %H:%M:%S] Creating placeholders for output")
        ...
        /src/run_STAR.py \
            ...
            --output_dir star_out \
            --threads ${num_threads}
        echo $(date +"[%b %d %H:%M:%S] STAR complete!")
    }
#    /src/run_STAR.py \
#        star_index $fastq1_abs $fastq2_abs ${prefix} \
#        --output_dir star_out \
